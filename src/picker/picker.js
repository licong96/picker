...
export default class Picker extends EventEmitter {
  constructor(options) {
    ...
    document.getElementById(this.options.id).appendChild(this.pickerEl);
  ...
  _createWheel(wheelEl, i) {
    ...
    ((index) => {
      ...
      const wheelItem = [
        this.wheelEl[0].getElementsByClassName('wheel-item'),
        this.wheelEl[1].getElementsByClassName('wheel-item')
      ];
      const active = () => {
        const currentIndex = this.wheels[index].getSelectedIndex();
        const currentWheel = wheelItem[i];
        for (let j = 0; j < currentWheel.length; j++) {
          removeClass(currentWheel[j], 'active');
        }
        addClass(currentWheel[currentIndex], 'active');
      };
      let time = null;
      this.wheels[index].on('scroll', () => {
        time && clearTimeout(time);
        time = setTimeout(active, 10);
      });
      active();
    })(i);
